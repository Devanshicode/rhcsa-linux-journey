...
What surprised me was the `--parents` option. It tells `cp` to recreate the complete path of the file inside the backup directory. For example, instead of copying only `tuned.conf`, it also recreates the `/etc/modprobe.d/` part. This makes the backup much more organized and easier to restore later.
...
### Q2: `rm -rf` has no undo — what does this teach you about how Linux trusts its users vs Windows which asks "Are you sure?"
...
After using Linux more, I understood that Linux follows a different philosophy. It assumes that if a user runs a command, they know what they are doing. The system trusts the user completely and does not add extra confirmation steps.
...
### Q3: `find` uses `-mtime` for time and `-name` for filename — how would you combine BOTH in one command to find `.conf` files modified in the last 7 days?
...
### Q4: In AWS, this task = an S3 backup script. How would the `find + cp` logic translate to `aws s3 cp` commands?
